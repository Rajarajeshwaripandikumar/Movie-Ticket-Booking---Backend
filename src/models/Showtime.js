  ...
  try {
    if (!this.endTime && this.durationMins) {
      this.endTime = new Date(
        new Date(this.startTime).getTime() + this.durationMins * 60000
      );
    }
...
showtimeSchema.methods.ensureSeatsInitialized = async function () {
  ...
  if (Array.isArray(scr.seats) && scr.seats.length > 0) {
    const seats = scr.seats.map((s, index) => {
      const seatType = s.seatType || "REGULAR";

      // derive a stable seatId from whatever Screen.seats has
      const rawSeatId =
        s.seatId || // preferred
        s.code || // e.g. "A1"
        s.label || // e.g. "A1"
        (s.row && (s.col || s.col === 0)
          ? `${s.row}${s.col}` // row "A", col 1 => "A1"
          : null) ||
        (typeof s.index === "number"
          ? `S${s.index + 1}`
          : `S${index + 1}`); // fallback: S1, S2...

      const overridePrice =
        this.pricingOverrides?.find((p) => p.seatType === seatType)?.price;
      const typePrice = scr.pricing?.[seatType];

      return {
        seatId: String(rawSeatId), // never undefined now
        row: s.row ?? (s.rowIndex != null ? String(s.rowIndex) : undefined),
        col: s.col ?? (s.colIndex != null ? Number(s.colIndex) : undefined),
        seatType,
        price: s.price ?? overridePrice ?? typePrice ?? this.basePrice,
      };
    });
  ...
  const result = this.seats.map((s) => {
    ...
    if (bookedSeatSet.has(seatId)) {
      return {
        ...
        price: s.price,
        availability: "BOOKED",
        bookingId: bookedSeatSet.get(seatId),
      };
    }
 ...
 */
showtimeSchema.statics.hasOverlap = async function (
  screenId,
  candidateStart,
  candidateEnd,
