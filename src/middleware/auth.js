...
const AUTH_TRUST_TOKEN =
  (process.env.AUTH_TRUST_TOKEN || "false").toLowerCase() === "true";

// timeout for DB fetch (ms)
const FETCH_USER_TIMEOUT_MS = Math.max(
  200,
  Number(process.env.AUTH_USER_FETCH_TIMEOUT_MS || 1500)
);
...
async function loadUserFromDbWithTimeout(userId) {
  ...
  try {
    ...
    const p = User.findById(userId).select(
      "role theatreId theaterId email name"
    ).lean();

    const t = new Promise((_, rej) =>
      setTimeout(
        () => rej(new Error("user-fetch-timeout")),
    ...
    try {
      ...
      console.warn(
        "[Auth] loadUserFromDbWithTimeout fallback:",
        e?.message || e
      );
      return null;
    }
    ...
    console.warn(
      "[Auth] loadUserFromDbWithTimeout import failed:",
      err?.message || err
    );
    return null;
  }
...
export const requireAuth = (opts = {}) => {
  ...
  return async (req, res, next) => {
    // Let global CORS preflight middleware handle OPTIONS; just pass-through here
    if (req.method === "OPTIONS") {
      console.log("[requireAuth] OPTIONS", req.originalUrl, "→ pass-through");
      return next();
    }

    try {
      console.log("[requireAuth] ENTER", req.method, req.originalUrl);
      ...
      const isAnalytics =
        req.baseUrl && String(req.baseUrl).includes("/api/analytics");
      const isStream = req.path && String(req.path).includes("/stream");
      if (
        !token &&
        req.query?.token &&
      ...
      try {
        ...
        console.error(
          "[requireAuth] token verify failed:",
          err && (err.message || err)
        );
      ...
      if (!userId) {
        console.warn(
          "[requireAuth] token missing subject (sub/id) for",
          req.originalUrl
        );
        return res
          .status(401)
          .json({ message: "Token missing subject (sub/id)" });
      }
      ...
      const finalRole = normalizeRole(
        dbUser?.role ?? rawRoleFromToken ?? "USER"
      );
      const finalTheatreId =
        dbUser?.theatreId ?? dbUser?.theaterId ?? theatreFromToken ?? null;
...
export const requireRoles = (...allowedArgs) => {
  ...
  return (req, res, next) => {
    ...
    if (!res || typeof res.status !== "function") {
      console.error(
        "[Auth] requireRoles used incorrectly (res missing). Check route wiring."
      );
      return next(
        new Error(
          "requireRoles used incorrectly; ensure you're calling the factory and not invoking middleware at import time"
        )
      );
    }
    ...
    if (!req.user) {
      console.warn(
        "[Auth] requireRoles — no req.user for",
        req.method,
        req.originalUrl
      );
