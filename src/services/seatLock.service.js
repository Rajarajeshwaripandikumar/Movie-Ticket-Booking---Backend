...
function normalizeSeat(seat) {
  ...
  // object form {row, col} or {seatId/seat/label}
  if (typeof seat === "object" && seat !== null) {
    // If a seatId / label is provided, normalize that instead
    if (seat.seatId != null || seat.seat != null || seat.label != null) {
      const label = seat.seatId ?? seat.seat ?? seat.label;
      return normalizeSeat(String(label));
    }
...
export async function getUnavailableSeats(showtimeId) {
  ...
  // Confirmed bookings for this showtime
  const bookingsPromise = Booking.find(
    { showtime: stId, status: "CONFIRMED" },
    { seats: 1, _id: 0 }
  ).lean();

  const [locks, confirmedBookings] = await Promise.all([
    locksPromise,
    bookingsPromise,
  ]);

  // seats marked BOOKED/SOLD in showtime.seats (if present)
...
export async function lockSeats({ showtimeId, seats = [], userId }) {
  ...
  try {
    ...
    // If it's a duplicate key error from the unique (showtime, seat) index,
    // recompute actual conflicts for better UX
    if (err && err.code === 11000) {
      try {
        const conflictedLocks = await SeatLock.find({
          showtime: stId,
          seat: { $in: normalizedSeats },
          lockedUntil: { $gt: new Date() },
          lockedBy: { $ne: userId },
        ...
        return {
          ok: false,
          conflicts: conflictedLocks.map((x) => normalizeSeat(x.seat)),
          error: "Some seats are already locked by another user.",
        };
      } catch (e2) {
        console.error("[seatLock] second-pass conflict lookup failed:", e2);
      }
    }
...
export async function releaseSeats({ showtimeId, seats = [], userId }) {
  if (!showtimeId) throw new Error("showtimeId required");
  if (!Array.isArray(seats) || seats.length === 0) {
    return { ok: false, error: "seats array required" };
  }
