...
import { requireAuth, requireRoles } from "../middleware/auth.js"; // 👈 change here
...
if (!CLOUDINARY_ENABLED) {
  console.warn(
    "[Movies] Cloudinary not fully configured (CLOUDINARY_CLOUD_NAME/API_KEY/API_SECRET). " +
      "Poster uploads will be skipped in this environment."
  );
}
...
function sanitizeCastArray(inputArr) {
  ...
  return inputArr
    ...
    .filter(
      (x) =>
        x &&
        (String(x.name).trim().length > 0 ||
          String(x.character).trim().length > 0)
    );
}
...
router.get("/", async (req, res) => {
  try {
    ...
    const { q, genre, page = 1, limit = 20, onlyInTheaters, status } =
      req.query;
    const filter = {};
    ...
    if (onlyInTheaters === "true" || onlyInTheaters === true)
      filter.inTheaters = true;
...
router.get("/:id", async (req, res) => {
  try {
    ...
    if (!isValidId(id)) {
      return res
        .status(400)
        .json({ ok: false, message: "Invalid movie id" });
    }
...
adminRouter.get("/list", async (req, res) => {
  try {
    ...
    if (role === "THEATER_ADMIN" || role === "THEATRE_ADMIN") {
      if (!req.user.theater && !req.user.theatreId) {
        return res.status(403).json({
          ok: false,
          message: "Your account is not linked to a theater",
        });
      }
      q.theater = req.user.theater || req.user.theatreId;
    }
...
adminRouter.post("/", async (req, res) => {
  console.log("[Movies][Admin] POST /admin hit", {
    user: req.user && (req.user.email || req.user.id || req.user.sub),
  });
  ...
  try {
    ...
    const doc = {
      ...
      runtimeMinutes: payload.runtimeMinutes
        ? Number(payload.runtimeMinutes)
        : payload.runtime
        ? Number(payload.runtime)
        : null,
    ...
    if (role === "THEATER_ADMIN" || role === "THEATRE_ADMIN") {
      ...
      doc.theater = req.user.theater || req.user.theatreId;
...
adminRouter.put("/:id", async (req, res) => {
  const { id } = req.params;
  console.log("[Movies][Admin] PUT /admin/:id incoming", {
    id,
    user: req.user && (req.user.email || req.user.id || req.user.sub),
  });

  res.setTimeout(10000, () => {
    console.error("[Movies][Admin] PUT /admin/:id timed out at server side", {
      id,
    });
    if (!res.headersSent) {
      return res.status(503).json({
        ok: false,
        message: "Movie update timed out on server",
      });
    }
  });
  ...
  try {
    if (!isValidId(id)) {
      console.warn("[Movies][Admin] invalid id", { id });
      return res
        .status(400)
        .json({ ok: false, message: "Invalid movie id" });
    }
    ...
    console.log("[Movies][Admin] existing movie loaded?", !!existing);
    ...
    const role = String(req.user?.role || "").toUpperCase();
    if (role === "THEATER_ADMIN" || role === "THEATRE_ADMIN") {
      const userTheater = req.user.theater || req.user.theatreId;
      if (!userTheater) {
        console.warn("[Movies][Admin] theater admin without theater");
        return res.status(403).json({
          ok: false,
          message: "Your account is not linked to a theater",
        });
      }
      if (!existing.theater || String(existing.theater) !== String(userTheater)) {
        console.warn("[Movies][Admin] forbidden movie ownership", {
          movieTheater: existing.theater,
          userTheater,
        });
        return res.status(403).json({
          ok: false,
          message: "Forbidden — this movie is not owned by your theater",
        });
      }
    }

    const body = req.body || {};
    console.log("[Movies][Admin] body keys:", Object.keys(body || {}));

    const genres = body.genres
      ? normalizeArrayField(body.genres)
      : existing.genres || [];
    const rawCast = body.cast
      ? normalizeArrayField(body.cast)
      : existing.cast || [];
    const rawCrew = body.crew
      ? normalizeArrayField(body.crew)
      : existing.crew || [];
    const languages = body.languages
      ? normalizeArrayField(body.languages)
      : existing.languages || ["English"];

    const payload = {
      title: body.title ?? existing.title,
      description:
        body.description ?? body.synopsis ?? existing.description,
    ...
    if (role === "THEATER_ADMIN" || role === "THEATRE_ADMIN") {
      payload.theater = req.user.theater || req.user.theatreId;
    ...
    console.log("[Movies][Admin] update payload prepared");

    let updated;
    try {
      updated = await Movie.findByIdAndUpdate(id, payload, {
        new: true,
        runValidators: true,
      }).lean();
      console.log("[Movies][Admin] update done, updated?", !!updated);
    } catch (validationErr) {
      console.error(
        "[Movies][Admin] Validation error on update:",
        validationErr
      );
    ...
    if (!updated) {
      console.warn("[Movies][Admin] update returned null");
...
adminRouter.delete("/:id", async (req, res) => {
  console.log("[Movies][Admin] DELETE /admin/:id hit", {
    id: req.params.id,
    user: req.user && (req.user.email || req.user.id || req.user.sub),
  });
  ...
  try {
    ...
    if (role === "THEATER_ADMIN" || role === "THEATRE_ADMIN") {
      ...
      if (
        !existing.theater ||
        String(existing.theater) !== String(userTheater)
      ) {
        return res.status(403).json({
          ok: false,
          message:
            "Forbidden — this movie is not owned by your theater",
        });
      }
    }
    ...
    if (deleted.posterPublicId && CLOUDINARY_ENABLED) {
      try {
        await cloudinary.uploader.destroy(deleted.posterPublicId);
        console.log(
          "[Cloudinary] deleted poster:",
          deleted.posterPublicId
        );
      } catch (err) {
        console.warn(
          "[Cloudinary] Failed to delete poster:",
          err?.message || err
        );
      }
    }
...
// 👇 THIS WAS THE HANG: you had requireAuth without calling it.
router.use(
  "/admin",
  requireAuth(), // call the factory
...
router.use((err, _req, res, next) => {
  ...
  if (err instanceof multer.MulterError) {
    if (err.code === "LIMIT_FILE_SIZE")
      return res
        .status(413)
        .json({ ok: false, message: "File too large (max 8MB)" });
    return res.status(400).json({
      ok: false,
