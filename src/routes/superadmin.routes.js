...
async function createAdmin(req, res) {
  try {
    ...
    // name, email, password required; theatreId optional
    ...
    if (theatreId && !isObjId(theatreId)) {
      return res
        .status(400)
        .json({ code: "INVALID_THEATER_ID", message: "Invalid theatreId" });
    }
    ...
    if (existingByEmail) {
      return res
        .status(409)
        .json({ code: "EMAIL_TAKEN", message: "Email already exists" });
    }

    // if theatreId provided, validate it & ensure it doesn't already have an admin
    if (theatreId) {
      ...
      if (!theatre) {
        return res
          .status(404)
          .json({ code: "THEATER_NOT_FOUND", message: "Theater not found" });
      }
      ...
      if (existingAdmin) {
        return res.status(409).json({
          code: "THEATER_ALREADY_HAS_ADMIN",
          message: "Theater already has an admin",
        });
      }
    }

    // Create user: set passwordHash raw; pre-save hook will hash once.
    const newAdmin = await User.create({
      name,
      email,
      passwordHash: String(password),
      role: ROLE.THEATER_ADMIN || "THEATER_ADMIN",
    ...
    return res.status(201).json({
      ok: true,
      message: "Theater admin created successfully",
      admin: {
        id: newAdmin._id,
        name: newAdmin.name,
        email: newAdmin.email,
        role: newAdmin.role,
        theatreId: newAdmin.theatreId || null,
        isActive: newAdmin.isActive ?? true,
        createdAt: newAdmin.createdAt,
      },
    });
  } catch (err) {
    console.error("[SuperAdmin] create theater admin error:", err);
    ...
    return res
      .status(500)
      .json({ code: "INTERNAL", message: "Failed to create theater admin" });
  }
}

async function listAdmins(_req, res) {
  try {
    const admins = await User.find({ role: "THEATER_ADMIN" })
    ...
    console.error("[SuperAdmin] list theater admins error:", err);
    return res
      .status(500)
      .json({ code: "INTERNAL", message: "Failed to load theater admins" });
  }
}
...
async function getAdmin(req, res) {
  try {
    ...
    const admin = await User.findOne({ _id: id, role: "THEATER_ADMIN" })
      .populate("theatreId", "name city")
      .select("name email role theatreId isActive createdAt")
      .lean();
    ...
    if (!admin)
      return res
        .status(404)
        .json({ code: "NOT_FOUND", message: "Theater admin not found" });
    return res.json({ ok: true, admin });
  } catch (err) {
    console.error("[SuperAdmin] get theater admin error:", err);
    res
      .status(500)
      .json({ code: "INTERNAL", message: "Failed to load theater admin" });
  }
}
  ...
  try {
    ...
    if (typeof email === "string") {
      ...
      if (taken) {
        return res
          .status(409)
          .json({ code: "EMAIL_TAKEN", message: "Email already in use" });
      }
      update.email = e;
    }
    ...
    if (typeof theatreId !== "undefined") {
      ...
      update.theatreId = theatreId || undefined;
    }

    if (typeof isActive === "boolean") update.isActive = isActive;

    const doc = await User.findOneAndUpdate(
      { _id: id, role: "THEATER_ADMIN" },
      { $set: update },
    ...
    console.error("[SuperAdmin] update theater admin error:", err);
    if (err?.code === 11000) {
      return res
        .status(409)
        .json({ code: "EMAIL_TAKEN", message: "Email already in use" });
    ...
    res
      .status(500)
      .json({ code: "INTERNAL", message: "Failed to update theater admin" });
  }
}
...
async function updateAdminStatus(req, res) {
  try {
    ...
    if (typeof isActive !== "boolean") {
      return res.status(400).json({
        code: "BAD_REQUEST",
        message: "isActive boolean required",
      });
    }
    ...
    console.error("[SuperAdmin] status theater admin error:", err);
    res
      .status(500)
      .json({ code: "INTERNAL", message: "Failed to update status" });
  }
}
...
async function deleteAdmin(req, res) {
  try {
    ...
    return res.json({ ok: true, message: "Theater admin deleted", id });
  } catch (err) {
    console.error("[SuperAdmin] delete theater admin error:", err);
    res.status(500).json({
      code: "INTERNAL",
      message: "Failed to delete theater admin",
    });
  }
}
...
// Legacy create route kept for backward compatibility with your React page
router.post(
...
// New conventional routes
adminBases.forEach((base) => {
  router.post(`/${base}`, requireAuth(), requireRole(["SUPER_ADMIN"]), createAdmin);
  router.get(`/${base}`, requireAuth(), requireRole(["SUPER_ADMIN"]), listAdmins);
  router.get(`/${base}/:id`, requireAuth(), requireRole(["SUPER_ADMIN"]), getAdmin);
  router.put(`/${base}/:id`, requireAuth(), requireRole(["SUPER_ADMIN"]), updateAdmin);
  router.patch(
    `/${base}/:id/status`,
    requireAuth(),
    requireRole(["SUPER_ADMIN"]),
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ ok: false, message: "Failed to load theaters" });
    }
  }
);
  ...
  async (req, res) => {
    try {
      ...
      if (!name || !city) {
        return res
          .status(400)
          .json({ ok: false, message: "name & city required" });
      }
      ...
      return res
        .status(500)
        .json({ ok: false, message: "Failed to create theater" });
    }
  }
);
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ ok: false, message: "Failed to update theater" });
    }
  }
);
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ ok: false, message: "Failed to delete theater" });
    }
  }
);
  ...
  async (req, res) => {
    try {
      ...
      if (!isObjId(showtimeId)) {
        return res
          .status(400)
          .json({ code: "BAD_ID", message: "Invalid showtimeId" });
      }
      if (
      ...
      if (!doc)
        return res
          .status(404)
          .json({ code: "NOT_FOUND", message: "Showtime not found" });
      return res.json({
        ok: true,
      ...
      return res
        .status(500)
        .json({ code: "INTERNAL", message: "Failed to update pricing" });
    }
  }
);
