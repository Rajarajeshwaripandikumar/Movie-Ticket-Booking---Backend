...
function isAdminRole(role) {
  ...
  // Accept both spellings for backward compatibility; canonical in code/DB is THEATER_ADMIN
  return (
    r === "SUPER_ADMIN" ||
    r === "THEATER_ADMIN" ||
    r === "THEATRE_ADMIN" ||
...
function safeUserPayload(userDoc) {
  ...
  return {
    ...
    // expose theatre linkage so frontend can use it
    theatreId: u.theatreId || u.theaterId || null,
    theaterId: u.theatreId || u.theaterId || null,
...
async function signTokenForUser(user) {
  ...
  // include role & theatreId for downstream middlewares
  const payload = {
...
router.post("/login", async (req, res) => {
  try {
    ...
    // Admins can technically log in here, but you prefer explicit /admin/login
    if (isAdminRole(user.role)) {
      // you could enforce admin-only login via /admin/login by returning 403 here
 ...
 * Body: { email, password }
 * Accepts THEATER_ADMIN / THEATRE_ADMIN / SUPER_ADMIN / ADMIN
 */
router.post("/admin/login", async (req, res) => {
  try {
    ...
    if (!isAdminRole(user.role)) {
      return res
        .status(403)
        .json({ message: "Unauthorized — not an admin" });
    }
...
router.post("/forgot-password", async (req, res) => {
  try {
    ...
    const note =
      sent.previewUrl && process.env.NODE_ENV !== "production"
  ...
  try {
    ...
    mailer
      .sendEmail({
        ...
        html: `<p>Hello ${
          user.name || "User"
        },</p><p>Your password was successfully changed. If you did not do this, contact support immediately.</p>`,
