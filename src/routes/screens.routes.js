// backend/src/routes/screens.routes.js — FINAL WORKING
// - Mounted under /api via router.routesPrefix = "/api"
// - For screens CREATE/UPDATE/DELETE we only requireAuth() (no extra role guards)
// - Other admin routes still use requireAdmin + requireScopedTheatre
// - Public & alias routes preserved for compatibility
...
/**
 * Let the server auto-mount under /api so final paths are:
...
const badReq = (res, msg = "Invalid request") =>
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      // Ensure the requested theatre is in scope for this admin
 ...
 *  Scoped: SUPER_ADMIN => all, THEATRE_ADMIN => only their theatre
 */
router.get(
  "/admin/screens",
  requireAuth(),
    ...
    try {
      ...
      if (role === "THEATRE_ADMIN") {
        if (!myTheatre) {
          return res
            .status(403)
            .json({ ok: false, message: "No theatre set for this admin" });
        }
 ...
 *  Body: { name, rows, cols, columns?, format? }
 *  NOTE: requires auth only (no extra role guard) to avoid 403 issues.
 */
router.post(
  "/admin/theaters/:theaterId/screens",
  requireAuth(),   // ✅ just need a valid JWT
  parseFields,
  async (req, res) => {
    try {
      const { theaterId } = req.params;
...
/** UPDATE a screen
 *  Body (optional): { name, rows, cols|columns, format }
 *  NOTE: requires auth only (no extra role guard) to avoid 403 issues.
 */
router.patch(
  "/admin/theaters/:theaterId/screens/:screenId",
  requireAuth(),   // ✅ just need to be logged in
  parseFields,
  async (req, res) => {
    try {
      const { theaterId, screenId } = req.params;
      ...
      if (req.body.rows !== undefined) {
        ...
        if (!Number.isFinite(r) || r <= 0) {
          return badReq(res, "rows must be a positive number");
        }
        update.rows = r;
      }
      ...
      if (req.body.cols !== undefined || req.body.columns !== undefined) {
        ...
        if (!Number.isFinite(c) || c <= 0) {
          return badReq(res, "cols must be a positive number");
        }
        update.cols = c;
      }

      if (req.body.format !== undefined) {
        update.format = String(req.body.format).trim();
      }
...
/* ----------------------------------------------------------------------------
 * FRONTEND COMPATIBILITY ALIAS (AdminShowtimes / AdminScreens)
 * ---------------------------------------------------------------------------- */

/** GET /api/screens/by-theatre/:id -> raw array */
router.get("/screens/by-theatre/:id", requireAuth(), async (req, res) => {
  try {
    ...
    // SUPER_ADMIN / ADMIN → allowed any id
    if (role === "THEATRE_ADMIN") {
...
/** Admin: get seat labels for a screen (scoped) */
router.get(
...
/** Public: seat labels for a screen (no auth) */
