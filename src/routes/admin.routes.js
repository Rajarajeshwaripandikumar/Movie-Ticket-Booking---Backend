...
/**
 * GET /api/admin/debug/me
 * Any authenticated user; returns whatever requireAuth attached as req.user
 */
...
/**
 * GET /api/admin/me
 * Returns minimal profile for the current admin.
...
router.post(
  ...
  async (req, res) => {
    try {
      const { name, city, address, contactEmail, contactPhone } =
        req.body || {};
      if (!name || !city)
        return res
          .status(400)
      ...
      if (exists)
        return res
          .status(409)
          .json({ message: "Theater already exists in this city" });
      ...
      res
        .status(500)
        .json({ message: "Failed to create theater", error: e.message });
    }
  }
);
  ...
  async (req, res) => {
    try {
      ...
      const theater = await Theater.findOne({
        theaterAdmin: req.user.id,
      });
      if (!theater)
        return res
          .status(404)
          .json({ message: "No theater assigned to you" });
      return res.json([theater]);
    } catch (e) {
      console.error("[Admin] load theaters error:", e);
      res
        .status(500)
        .json({ message: "Failed to load theaters", error: e.message });
    }
  }
);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      if (
        req.user.role === "THEATER_ADMIN" &&
        String(t.theaterAdmin || "") !== String(req.user.id)
      ) {
      ...
      res
        .status(500)
        .json({ message: "Failed to fetch theater", error: e.message });
    }
  }
);
 ...
 * SUPER_ADMIN: can update any theater
 * THEATER_ADMIN: can update only their assigned theater
 */
router.put(
  "/theaters/:id",
  requireRole("THEATER_ADMIN", "SUPER_ADMIN"),
  async (req, res) => {
    try {
      ...
      // If THEATER_ADMIN, ensure this is their own theater
      if (req.user.role === "THEATER_ADMIN") {
        const theater = await Theater.findById(id).lean();
        if (
          !theater ||
          String(theater.theaterAdmin || "") !== String(req.user.id)
        ) {
          return res
            .status(403)
            .json({ message: "Forbidden: not your theater" });
        }
      }

      const updated = await Theater.findByIdAndUpdate(id, update, {
        new: true,
        runValidators: true,
      });
      ...
      console.error("[Admin] update theater error:", e);
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Failed to delete theater", error: e.message });
    }
  }
);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Failed to assign admin", error: e.message });
    }
  }
);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Failed to load showtimes", error: e.message });
    }
  }
);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!movie || !screenId || !startTime)
        return res
          .status(400)
          .json({ message: "movie, screen, startTime required" });
      ...
      if (req.user.role === "THEATER_ADMIN") {
        ...
        if (!t || String(t._id) !== String(screen.theater)) {
          return res
            .status(403)
            .json({ message: "Forbidden: you don't manage this theater" });
        }
      }
      ...
      res
        .status(500)
        .json({ message: "Failed to create showtime", error: e.message });
    }
  }
);
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Failed to update showtime", error: e.message });
    }
  }
);
  ...
  async (req, res) => {
    try {
      ...
      if (pricingOverrides !== undefined)
        update.pricingOverrides = pricingOverrides;
      ...
      res
        .status(500)
        .json({ message: "Failed to update pricing", error: e.message });
    }
  }
);
  ...
  async (req, res) => {
    try {
      ...
      if (req.user.role === "THEATER_ADMIN") {
        ...
        const showtimeIds = await Showtime.find({
          theater: t._id,
        }).distinct("_id");
      ...
      const revenue = bookings.reduce(
        (sum, b) => sum + (Number(b.totalAmount || b.amount || 0) || 0),
        0
      );
